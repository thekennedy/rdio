...
class RdioRadio:
  ...
  def next_track(self):
    ...
    while not track:
      ...
      if artist:
        ...
        if not track:
          self.debug("No tracks found for artist %s, adding to blacklist" % artist)
          artist_blacklist.append(artist)
      else:
        self.debug("Didn't find an artist")
        attempt_number = attempt_number + 1
        if attempt_number == 1:
          self.debug("Allowing base artist and artist repeats")
          artist_blacklist = list(set(artist_blacklist) - (set([base_artist]) | set(self._state['played_artists'])))
        if attempt_number == 2:
          self.debug("Allowing track repeats")
          artist_blacklist = []
  ...
  def _choose_artist(self, last_artist, user, artist_blacklist = None, depth = 1):
    self.debug("Choosing artist with last artist %s" % last_artist)
    ...
    self.debug("Chose artist: %s" % str(chosen_artist))
  ...
  def _candidate_artists(self, last_artist, user, artist_blacklist = None, artist_recurse_blacklist = None, depth = 1):
    self.debug("Finding candidate artists with last artist %s" % last_artist)
    ...
    if not user:
      self.debug("Candidate artists: %s" % str(allowed_related_artist_keys))
    ...
    self.debug("Related artists: %s, collection artists: %s, blacklist: %s" % (str(allowed_related_artist_keys), str(collection_artist_keys), str(artist_blacklist)))
    candidate_artist_keys = list(set(allowed_related_artist_keys) & set(collection_artist_keys))
    self.debug("Candidate artists: %s" % str(candidate_artist_keys))
    ...
    if not candidate_artist_keys and depth < self._RELATED_ARTIST_DEPTH:
      ...
      self.debug("Recursing related artists %s, recurse blacklist: %s" % (str(recurse_artists), str(artist_recurse_blacklist)))
